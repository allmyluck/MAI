...
void Lowercase(TKey* str) {
    int len = LengthOfByte(str);
    for (int i = 0; i < len; ++i) {
...
int main(void) {
    ...
    char input[257];
    TPatricia* trie = new TPatricia; 
    while((std::cin >> input)) {
        if(input[0] == '+') {
            char* nkey = new char[max_size];
            std::cin >> nkey;
            Lowercase(nkey);
            TData get;
            std::cin >> get;
            if(trie->Insert(nkey,get) == true) {
                std::cout << "OK\n";
            } else {
                delete [] nkey; 
                std::cout << "Exist\n";
            ...
            Lowercase(input);
            TNode* cnode;
            cnode = trie->Find(input);
            if(cnode == nullptr) {
                std::cout << "NoSuchWord\n";
            } else {
                std::cout << "OK: " << cnode->GetData() << "\n";
            }
        }
    }
    trie->~TPatricia();
