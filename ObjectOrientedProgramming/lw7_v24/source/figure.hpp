...
public:
    Octagon() : a(0,0), R(0), id("o") 
    {}
    Octagon(T x1, T y1, T Rad) : a(x1,y1), R(Rad),id("o")
    {
        if(Rad <= 0) {
...
template <class T>
class OctagonFactory : public FigureFactory<T> {
public:
    using ptr = std::shared_ptr<Figure<T>>;
    std::shared_ptr<Figure<T>> CreateFigure(std::istream& in) {
        T x1, y1, R;
        in >> x1; in >> y1; in >> R;
        return ptr(std::make_shared<Octagon<T>>(x1,y1,R));
    }
};
