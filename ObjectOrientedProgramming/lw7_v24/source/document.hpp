...
class Document {
public:
    Document(std::string docname) : buffer(), name(docname),trfactory(), ocfactory(), sqfactory()
    {}
    ...
    std::shared_ptr<Figure<T>> Insert(FigureType type) {
        switch(type) {
            ...
            case FigureType::octagon:
                ...
                buffer.push_back(ocfactory.CreateFigure(std::cin));
                break;
        }
        return buffer.back();
    }
    ...
    bool ReadFromFile(std::string filename) {
        ...
        while(!file.eof()) {
            ...
            file >> type;
            if(type == "o") {
                buffer.push_back(ocfactory.CreateFigure(file));
            } else if(type == "t") {
