...
Разработать класс на С++, описывающий такие множества. Должны быть методы
    ...
    проверка корректности: начало<=конец : есть(начало => конец)

    нормализация множества отрезков: отрезки не пересекаются и упорядочены по возрастанию (т.е. произвести операции объединения).
